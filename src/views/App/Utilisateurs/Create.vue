        ...
        <div class="p-5">
            ...
            <template v-else>
                <form action="" class="bg-white w-2/3 p-2 mx-auto rounded-lg" @submit.prevent="createUser">
                    ...
                    <div class="lg:mb-3" v-if="user.roles_id?.find(id => id == createData.roles?.find(role => role.label == 'responsable').id) != undefined">
                    ...
                    <div class="lg:mb-3" v-if="user.roles_id?.find(id => id == createData.roles?.find(role => role.label == 'responsable').id) != undefined" >
                        ...
                        <select id="filiere" v-model="user.filiere_id"
                            class="lg:border lg:rounded lg:w-full h-10 lg:h-10 lg:p-2 focus:outline-sky-600">
                            <option v-for="filiere in createData.filieres" :value="filiere.id" :key="filiere.id">{{filiere.label}} ({{filiere.code}})</option>
                        </select>
                    </div>
                    <div class="lg:mb-3" v-if="user.roles_id?.find(id => id == createData.roles?.find(role => role.label == 'responsable').id) != undefined">
                        <label for="year" class="lg:font-semibold lg:block lg:mb-1">Year</label>
                        <select id="year" v-model="user.year_id" class="lg:border lg:rounded lg:w-full h-10 lg:h-10 lg:p-2 focus:outline-sky-600">
                    ...
                    <div class="lg:mb-3">
                        ...
                        <input type="tel" v-model="user.telephone" id="telephone" pattern="\+229-[0-9]{8}" placeholder="+229-XXXXXXXX"
                            class="lg:border lg:rounded lg:w-full h-10 lg:h-10 lg:p-2 focus:outline-sky-600">
                    </div>
                    <div class="lg:mb-3">
