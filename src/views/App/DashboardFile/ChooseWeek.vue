            ...
            <template v-if="step=='weeks'">
                ...
                <div class="grid lg:grid-cols-5 gap-5">
                    <div v-for="week in weeks" :key="week.id"
                        class="h-24 border rounded bg-white hover:border-sky-600 hover:cursor-pointer flex justify-around flex-col px-1" @click="chooseWeek(week.id)">
                        <p class="text-center">Semaine du <span class="font-semibold">{{ week.start }}</span> au <span class="font-semibold">{{ week.end }}</span></p>
                    </div>
                </div>
            </template>
